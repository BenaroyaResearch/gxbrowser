...
d3.chart.box = function() {
  var width = 1,
      ...
      colors = null,
      showPoints = false,
      funcMouseover = function(x,v,i) {},
      funcMouseout = function() {},
      options = { strokeWidth: 1, highlightOpacity: 0.6, pointWidth: 3 };
  ...
  function box(g) {
    g.each(function(d, i) {
      if (d.length > 0)
      {
        ...
        var groupColor = colors[i] ? colors[i] : "#000000";
        ...
        // Update whiskers.
        var whiskerWidth = width * 0.8;
        var whiskerOffset = (width - whiskerWidth);
        ...
        if (showPoints)
        {
          var highlight = function(point) {
            var oldPoint = point;
            var parentNode = point.parentNode;
            parentNode.removeChild(point);
            parentNode.appendChild(oldPoint);
            d3.select(point)
              .attr("r", options.pointWidth+2)
              .style("fill", "#FFFFFF")
              .style("stroke-width", options.strokeWidth * 2);
          };
          var reset = function(point) {
            d3.select(point)
              .attr("r", options.pointWidth)
              .style("fill-opacity", 1)
              .style("fill", groupColor)
              .style("stroke-width", options.strokeWidth);
          };
          var xPoints = {};
          var randomizeX = function(v) {
            if (xPoints[v])
            {
          ...
          var scatterPlot = g.selectAll("scatter.plot")
            .data(d)
            .enter().append("svg:circle")
            .attr("r", options.pointWidth)
            .attr("cx", randomizeX)
            .attr("cy", function(v) { return x1(v); } )
            .style("fill", groupColor)
            .style("stroke", groupColor)
            .style("stroke-width", options.strokeWidth)
            .on("mouseover", function(v,idx) { highlight(this); funcMouseover(i,v,idx); })
            .on("mouseout", function() { reset(this); funcMouseout; });
        }
  ...
  box.options = function(x) {
    if (!arguments.length) return options;
    $.extend(options, x);
    return box;
  };

  box.funcMouseover = function(x) {
    if (!arguments.length) return funcMouseover;
    funcMouseover = x;
    return box;
  };

  box.funcMouseout = function(x) {
    if (!arguments.length) return funcMouseout;
    funcMouseout = x;
    return box;
  };
