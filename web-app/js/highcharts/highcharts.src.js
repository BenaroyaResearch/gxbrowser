...
function Chart (options, callback) {
	...
	function Axis (chart, options) {
        ...
        var totalColumns = 0, groupColumnCount = [];
        each(chart.series, function(s) {
          each(s.data, function(d, i) {
              if (groupColumnCount[i] == null)
              {
                groupColumnCount[i] = 1;
                totalColumns++;
              }
              else
              {
        ...
        var leftPadding = 5, borderWidth = defaultPlotOptions.column.borderWidth, shadowWidth = 2, columnPointWidth = chart.options.plotOptions.series.pointWidth;
//        var workableChartWidth = chart.plotSizeX - (leftPadding * 2);
        var workableChartWidth = chart.plotSizeX - (leftPadding * 2) - ((groupColumnCount.length - 1) * 10);
//        var singleColumnPaddedWidth = defined(columnPointWidth) ? columnPointWidth : Math.max(10, Math.floor(workableChartWidth / totalColumns));
//        var singleColumnPaddedWidth = Math.max(1, Math.floor(workableChartWidth / totalColumns));
        var singleColumnPaddedWidth = Math.max(1, (workableChartWidth / totalColumns));
        if (singleColumnPaddedWidth < 10)
        {
//          singleColumnPaddedWidth = 10;
          borderWidth = shadowWidth = 0;
        }
        var totalBarWidth = (singleColumnPaddedWidth * totalColumns) - defaultPlotOptions.column.columnPadding;
        var spaceWidth = Math.max(10, Math.floor((workableChartWidth - totalBarWidth) / (groupColumnCount.length - 1)));
        if (singleColumnPaddedWidth < 10)
        {
          spaceWidth = 10;
        }
        var groupWidths = [], groupOffsets = [];
        each(groupColumnCount, function(v, i) {
          groupWidths[i] = (singleColumnPaddedWidth * v) - defaultPlotOptions.column.columnPadding;
          groupOffsets[i] = (i == 0) ? leftPadding : groupOffsets[i-1] + groupWidths[i-1] + spaceWidth;
        });
        ...
        else
        {
          leftPadding = -1;
				...
				x = horiz ? plotLeft + groupOffsets[pos] + (groupWidths[pos] / 2) + leftPadding :
...
var ColumnSeries = extendClass(Series, {
	...
	translate: function() {
      ...
      groupColumnCount = [],
		...
		each(chart.series, function(otherSeries) {
          ...
          each(otherSeries.data, function(d, i) {
              if (groupColumnCount[i] == null)
              {
                groupColumnCount[i] = 1;
                totalColumns++;
    ...
    var categoryWidth = mathAbs(
				data[1] ? data[closestPoints].plotX - data[closestPoints - 1].plotX :
				chart.plotSizeX / ((categories && categories.length) || 1)
			);
    var leftPadding = 5, shadowWidth = 2;
    var workableChartWidth = chart.plotSizeX - (leftPadding * 2) - ((groupColumnCount.length - 1) * 10);
//    var singleColumnPaddedWidth = defined(options.pointWidth) ? options.pointWidth : Math.max(10, Math.floor(workableChartWidth / totalColumns));
//    var singleColumnPaddedWidth = Math.max(1, Math.floor(workableChartWidth / totalColumns));
    var singleColumnPaddedWidth = Math.max(1, (workableChartWidth / totalColumns));
    if (singleColumnPaddedWidth < 10)
    {
...
//      singleColumnPaddedWidth = 10;
    ...
    var singleColumnWidth = singleColumnPaddedWidth - options.columnPadding - (borderWidth * 2) - shadowWidth;
    var totalBarWidth = (singleColumnPaddedWidth * totalColumns) - options.columnPadding;
    var spaceWidth = Math.max(10, Math.floor((workableChartWidth - totalBarWidth) / (groupColumnCount.length - 1)));
    if (singleColumnPaddedWidth < 10)
    {
      spaceWidth = 10;
    ...
    var groupWidths = [], groupOffsets = [];
    each(groupColumnCount, function(v, i) {
      groupWidths[i] = (singleColumnPaddedWidth * v) - options.columnPadding;
      groupOffsets[i] = (i == 0) ? leftPadding : groupOffsets[i-1] + groupWidths[i-1] + spaceWidth;
    });
    ...
    each(data, function(point, currentGroup) {
      var gOffSet = (currentGroup == 0) ? leftPadding : groupOffsets[currentGroup];
      var pOffset = singleColumnPaddedWidth * series.columnIndex;
      var plotY = point.plotY,
				yBottom = point.yBottom || translatedThreshold,
				barX = gOffSet + pOffset,// + pointXOffset,
			...
			extend(point, {
				barX: barX,
				barY: barY, 
				barW: singleColumnWidth,//pointWidth,
				barH: barH
			});
